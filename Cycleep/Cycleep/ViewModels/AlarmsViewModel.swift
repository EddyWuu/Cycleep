...
final class AlarmsViewModel: ObservableObject {
    ...
    func addWakeAlarm(at time: Date, cycles: Int) {
        alarms.append(Alarm(time: time, label: "Wake up", kind: .wake, cycles: cycles))
    }

    /// Create a paired bedtime + wake-up alarm (Wake Up Time tab).
    func addSleepWakePair(sleepTime: Date, wakeTime: Date, cycles: Int) {
        let bedtime = Alarm(time: sleepTime, label: "Bedtime", kind: .sleep, cycles: cycles)
        let wake = Alarm(time: wakeTime, label: "Wake up", kind: .wake, cycles: cycles)
    ...
    func delete(at offsets: IndexSet, in displayed: [Alarm]) {
